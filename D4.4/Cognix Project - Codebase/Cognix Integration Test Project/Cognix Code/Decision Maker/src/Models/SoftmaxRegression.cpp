#include <Models/SoftmaxRegression.hpp>
#include <mlpack/core.hpp>
#include <mlpack/mlpack.hpp>

using namespace mlpack;
void DecisionMaking::SoftmaxRegression::Load(const std::string& path)
{
	mlpack::data::Load(path, "softmaxRegression", model);
}

void DecisionMaking::SoftmaxRegression::Save(const std::string& path)
{
	mlpack::data::Save(path, "softmaxRegression", model);
}

void DecisionMaking::SoftmaxRegression::Train(std::vector<double>& observations, size_t featureSize, std::vector<size_t>& labels, size_t numClasses)
{
	arma::mat data;
	FillArmaMat(observations, featureSize, data);
	model.Train(data, labels, numClasses);
}

double DecisionMaking::SoftmaxRegression::CrossValidate(size_t k, std::vector<double>& observations, size_t featureSize, std::vector<size_t>& labels, size_t numClasses, Metric metric)
{
	arma::mat data;
	FillArmaMat(observations, featureSize, data);

	if (k == 1) {
		switch (metric) {
		case BaseModel::Accuracy: {
			mlpack::SimpleCV<mlpack::SoftmaxRegression, mlpack::Accuracy> cv(0.2, data, labels, numClasses);
			return cv.Evaluate();
		}
		case BaseModel::F1:
		{
			auto cv = mlpack::SimpleCV<mlpack::SoftmaxRegression, mlpack::F1<mlpack::AverageStrategy::Macro>>(0.2, data, labels, numClasses);
			return cv.Evaluate();
		}
		case BaseModel::Precision:
		{
			SimpleCV<mlpack::SoftmaxRegression, mlpack::Precision<mlpack::AverageStrategy::Macro>> cv(0.2, data, labels, numClasses);
			return cv.Evaluate();
		}
		case BaseModel::Recall: {
			SimpleCV<mlpack::SoftmaxRegression, mlpack::Recall<mlpack::AverageStrategy::Macro>> cv(0.2, data, labels, numClasses);
			return cv.Evaluate();
		}
		}
	}
	else {
		switch (metric) {
		case BaseModel::Accuracy: {
			//mlpack::KFoldCV<mlpack::SoftmaxRegression, mlpack::Accuracy> cv(k, data, labels, numClasses);
			return 0;
		}
		case BaseModel::F1:
		{
			//mlpack::KFoldCV<mlpack::SoftmaxRegression, mlpack::F1<mlpack::AverageStrategy::Macro>> cv(k, data, labels, numClasses);
			return 0;
		}
		case BaseModel::Precision:
		{
			//mlpack::KFoldCV<mlpack::SoftmaxRegression, mlpack::Precision<mlpack::AverageStrategy::Macro>> cv(k, data, labels, numClasses);
			return 0;
		}
		case BaseModel::Recall: {
			//mlpack::KFoldCV<mlpack::SoftmaxRegression, mlpack::Recall<mlpack::AverageStrategy::Macro>> cv(k, data, labels, numClasses);
			return 0;
		}
		}
		return 0.0;
	}
}

std::vector<size_t> DecisionMaking::SoftmaxRegression::Predict(std::vector<double>& features, std::vector<size_t>& labels)
{
	arma::colvec queryData;
	FillArmaVec(features, queryData);

	auto result = model.Classify(queryData);

	return std::vector<size_t> { result };
}

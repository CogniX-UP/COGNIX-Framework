...
void LabStreamEEG::ConnectDevice() {
	...
	try {
		...
		auto channelMask = streamSetting.ChannelMask(eeg);
		auto activeChannelsCount = 0;
		for (bool m : channelMask) {
			if (m)
				activeChannelsCount++;
		}

		//basic data for stream to work
		streamInfo = lsl::stream_info(streamSetting.StreamName(), "EEG", activeChannelsCount, eeg.SampleRate(), lsl::cf_double64, "BioSemi");
		...
		auto appendChannel = [&chanInfo](std::string type, std::string unit) -> std::tuple<lsl::xml_element, lsl::xml_element> 
		{
			...
			return std::make_tuple(channel, hardware);
		};
		...
		//Order <Label, Type, Count> "Sync, Sync" -> "Trig, Trigger" -> "A1-H32, EEG, 256", "EX, EXG, 8", "AUX, AUX, ?", "AIB, Analog, ?"
		//Apply channels
		...
		for (int i = 0; i < channelMask.size(); i++) {
			...
			auto channHard = appendChannel(type, "microvolts");
			auto channel = std::get<0>(channHard);
			auto hardware = std::get<1>(channHard);
